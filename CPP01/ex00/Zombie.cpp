...
Zombie::~Zombie() {//Zombie::~Zombie() defines the destructor for the Zombie class. Destructors are special member functions which are called when an object of the class is destroyed (i.e., when it goes out of scope or is explicitly deleted).
...
void Zombie::announce() const {//void Zombie::announce() const defines a member function named announce that does not modify any member variables (as indicated by the const qualifier at the end of the method signature). This function allows a Zombie object to perform an action, which is to announce itself.
